// src/components/Logo.tsx (修正版 - preserveAspectRatio削除, ラッパー削除)
'use client';
...
export const Logo = () => {
    // ... (hooks, useEffect, offset calculations は変更なし) ...
    ...
    const waveVariants = {
        hidden: { opacity: 0, x: "-100%", scaleX: 1 }, // scaleX は 1 のまま
        slideIn: {
            opacity: 1,
            x: "0%",
            scaleX: 1,
        ...
        expandPlaceholder: { transition: { duration: 0.1 } },
        finalWiggle: {
            d: [wavePathBase, wavePathVariant1, wavePathVariant2, wavePathBase],
            transition: { duration: 5, ease: "easeInOut", repeat: Infinity }
        }
    };

    // 外側の div ラッパーを削除し、motion.svg を直接返す
    return (
        <motion.svg
            viewBox="0 0 800 250"
            width="100%"
            ...
            style={{
                ...
                // margin: '0 auto' // ← 中央揃えが必要なら復活させる
            }}
            // preserveAspectRatio="none" を削除 (デフォルトの均等スケールに戻す)
        >
            <defs>
            ...
            <motion.g
                fill={`url(#${gradientId})`}
                style={{ x: offsetX, y: offsetY }} // マウス追従
                variants={textVariants}
                initial="hidden"
                animate="visible"
            >
                 <motion.g
                    animate={{ skewY: [0, 1.5, -1.5, 0] }} // ゆらゆら
                 ...
                 >
                    <path // 文字パスデータ
                        ...
                        transform="translate(100 50) scale(2.5)" // 文字の位置とスケール
                    />
                 </motion.g>
            </motion.g>

...
interface Feeling {
  ...
  videoSrc?: string; // Keep for data structure consistency if needed
}
...
const initialFeelings: Feeling[] = [
  { id: 'beach_sand', name: 'Beach Sand', description: 'Warm sand flowing between your toes.', priceUSDC: 5.0, owned: true, category: 'nature', intensity: 6, attributes: { texture: 7, pressure: 4, temperature: 8 } },
  { id: 'athens_cobblestone', name: 'Athens Cobblestone', description: 'Ancient, smooth worn streets.', priceUSDC: 7.5, owned: true, category: 'urban', intensity: 8, attributes: { texture: 9, pressure: 7, temperature: 6 } },
  { id: 'forest_floor', name: 'Forest Floor', description: 'Soft, damp earth, moss, and leaves.', priceUSDC: 6.0, owned: false, category: 'nature', intensity: 7, attributes: { texture: 8, pressure: 5, temperature: 4 } },
  { id: 'grassy_field', name: 'Grassy Field', description: 'Tickling tall grass in a meadow.', priceUSDC: 4.5, owned: true, category: 'nature', intensity: 5, attributes: { texture: 6, pressure: 3, temperature: 7 } },
  { id: 'tokyo_asphalt', name: 'Tokyo Asphalt', description: 'Firm, pristine city streets.', priceUSDC: 8.0, owned: false, category: 'urban', intensity: 7, attributes: { texture: 5, pressure: 8, temperature: 6 } },
  { id: 'gravel_path', name: 'Gravel Path', description: 'Crunchy, uneven small stones.', priceUSDC: 5.5, owned: false, category: 'nature', intensity: 8, attributes: { texture: 9, pressure: 7, temperature: 5 } },
  { id: 'quantum_flow', name: 'Quantum Flow', description: 'Otherworldly flowing particles.', priceUSDC: 12.5, owned: false, category: 'abstract', intensity: 9, attributes: { texture: 9, pressure: 8, temperature: 7 } },
  { id: 'lunar_dust', name: 'Lunar Dust', description: 'Walking on the Moon in low gravity.', priceUSDC: 15.0, owned: false, category: 'abstract', intensity: 10, attributes: { texture: 10, pressure: 6, temperature: 2 } },
];
...
const formatDate = (timestamp: number): string => {
    ...
    try {
        // Intl.DateTimeFormat を使ってフォーマット試行
        return new Intl.DateTimeFormat(undefined, { dateStyle: 'short', timeStyle: 'short' }).format(date);
    } catch  { 
        
        console.warn("Intl.DateTimeFormat failed, using fallback.");
       
        const timeString = date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }); // hour12: false を追加推奨
...
export default function HapticPage() {
  ...
  const handleDisconnectDevice = () => {
    ...
    setCurrentActiveZone(null); setSelectedFeelingId(null);
  ...
  const handleSelectFeeling = (feelingId: string) => {
    ...
    if (deviceConnectionStatus !== 'connected') { setPurchaseStatus("Please connect your KAIKA device first."); setTimeout(() => setPurchaseStatus(null), 3000); return; }
    if (!feeling || !feeling.owned || loadingFeelingId || purchasingFeelingId) { return; }
    setLoadingFeelingId(feelingId); setPurchaseStatus(null); console.log(`Loading feeling: ${feeling.name}`);
    setTimeout(() => { setSelectedFeelingId(feelingId); setLoadingFeelingId(null); setIsPlaying(false); setCurrentActiveZone(null); console.log(`Selected feeling: ${feeling.name}`); }, 800);
  };
  ...
  const togglePlayback = () => {
    if (deviceConnectionStatus !== 'connected' || !selectedFeelingId) { if (deviceConnectionStatus !== 'connected') { openConnectModal(); } return; }
    const newState = !isPlaying; setIsPlaying(newState);
    console.log(newState ? `Playing feeling: ${selectedFeeling?.name}` : `Stopped feeling: ${selectedFeeling?.name}`);
    if (!newState) { setCurrentActiveZone(null); }
  };
  ...
  return (
    <div className={styles.main}>
      <AnimatedBackground feelingId={selectedFeelingId} />
      {isHapticOn && (
      ...
      <div className={styles.contentWrapper}>
        ...
        <div className={styles.contentPanel}>
           ...
           <section className={`${styles.section} ${styles.feelingsSection}`}>
              ...
              <ul className={styles.feelingsGrid}> {marketplaceFeelings.length > 0 ? marketplaceFeelings.map((feeling) => ( <motion.li key={feeling.id} className={`${styles.feelingGridItem} ${styles.notOwned} ${styles[feeling.category]}`} layout> <div className={styles.feelingContent}> <div className={styles.feelingInfo}> <div className={styles.feelingHeader}> <span className={styles.feelingName}>{feeling.name}</span> </div> <p className={styles.feelingDescription}>{feeling.description}</p> <div className={styles.feelingMetrics}> <div className={styles.metric}><span className={styles.metricLabel}>Intensity</span><span className={styles.metricValue}>{feeling.intensity}/10</span></div> </div> </div> <span className={styles.feelingPrice}>${feeling.priceUSDC.toFixed(2)} USDC</span> <div className={styles.buttons}> <AnimatePresence> {purchasingFeelingId === feeling.id ? ( <motion.div key="loader"><div className={styles.loader}></div></motion.div> ) : ( <motion.button key="buy-button" onClick={() => handlePurchaseFeeling(feeling.id)} className={`${styles.purchaseButton}`} disabled={!!purchasingFeelingId || !connected} > {`Buy for $${feeling.priceUSDC.toFixed(2)}`} </motion.button> )} </AnimatePresence> </div> </div> </motion.li> )) : ( <p className={styles.placeholderText} style={{ gridColumn: '1 / -1' }}> No items match the current filter in the marketplace. </p> )} </ul>
           </section>

           <section className={`${styles.section} ${styles.premiumSection}`}> <div className={styles.premiumBanner}> <h3>Premium Haptic Experiences</h3> <p>Coming soon - Join the waitlist for exclusive tactile sensations and early access.</p> <button className={styles.waitlistButton}>Join Waitlist</button> </div> </section>

           <section className={`${styles.section} ${styles.contractInfo}`}> <h2>Platform Information</h2> <div className={styles.contractGrid}> <div className={styles.contractItem}> <span className={styles.contractLabel}>NFT Contract</span> <span className={styles.contractValue} title="MockContractAddressHKAnft...dEvN">HKAn...dEvN</span> </div> <div className={styles.contractItem}> <span className={styles.contractLabel}>Total Experiences</span> <span className={styles.contractValue}>{availableFeelings.length}</span> </div> <div className={styles.contractItem}> <span className={styles.contractLabel}>Your Collection</span> <span className={styles.contractValue}>{ownedFeelings.length}</span> </div> <div className={styles.contractItem}> <span className={styles.contractLabel}>Network</span> <span className={styles.contractValue}>Solana Devnet</span> </div> </div> </section>
        </div>
      </div>
